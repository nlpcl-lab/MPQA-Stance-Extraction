...
class ACE2005Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        words, entities, postags, triggers, arguments = self.sent_li[idx], self.entities_li[
            idx], self.postags_li[idx], self.triggers_li[idx], self.arguments_li[idx]
        ...
        for w, e, p in zip(words, entities, postags):
            ...
            tokens_x.extend(tokens_xx), postags_x.extend(
                p), entities_x.extend(e), is_heads.extend(is_head)
...
def pad(batch):
    ...
    for i in range(len(tokens_x_2d)):
        ...
        postags_x_2d[i] = postags_x_2d[i] + \
            [0] * (maxlen - len(postags_x_2d[i]))
